...
| `--copy-comments [raw,doxygen]`                                  | Comments in the header file that are associated with a declaration will be copied to the generated bindings. If `raw` is used, comments that immediately precede a declaration will be copied as opaque text. If `doxygen` is used, the Doxygen comment will be converted to a proper JavaDoc comment. |
